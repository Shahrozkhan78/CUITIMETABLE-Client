...
// =============================================
const Reportit = (props) => {
  const showactionbtn=props.showactionbtnp;
    ...
    const [getbeforedate,setbeforedate]=useState(new Date())
...
setFilterData(defaultdata)
    ...
    const  date =today.getFullYear() +"-" +(today.getMonth() + 1) +"-" +today.getDate();
      const newdates= defaultdata.filter((obj)=>obj.Date===date ).map(items=>items)
...
const datestartfun2=(d1)=>{
   setFilterData(defaultdata)
 if(getbeforedate!=""&&d1!=""){
    
   
    var today = new Date(getbeforedate);
    var todayend = new Date(d1);  
  if(today<todayend){
    const newdates= defaultdata.filter((obj)=>new Date(obj.Date)>=today &&new Date(obj.Date)<=todayend ).map(items=>items)
    setFilterData(newdates);
      ...
      <Table sx={{ minWidth: 500 }} aria-label="custom pagination table">
         <TableHead className='bg-primary '>
          <TableRow>
            ...
            <TableCell className='text-light' style={{ display: showactionbtn ? " block" : "none" }} >Action</TableCell >
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={row.key}>
              ...
              <TableCell  scope="row" style={{ display: showactionbtn ? " block" : "none" }} >
                <div className='d-flex'>
                <button className='btn btn-warning d-flex mx-2  ' onClick={() => asktoconfirm(row._id)}><DeleteIcon className='text-light' /> </button>
